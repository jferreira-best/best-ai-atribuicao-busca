...
def classify_intent(query: str) -> dict:
    ...
    try:
        ...
        # =====================================================================
        # 6. HEURÍSTICAS PÓS-LLM (REGRAS DURAS)
        # =====================================================================
        q_norm = query.lower()

        # a) Qualquer coisa falando de "farol" (verde/amarelo/vermelho) -> Avaliação
        if "farol" in q_norm:
            if data.get("modulo") != "avaliacao":
                logging.debug(
                    "Override heurístico: detectado termo 'farol', "
                    "forçando modulo='avaliacao'."
                )
            data["modulo"] = "avaliacao"

            # Se a sub_intencao vier muito genérica ou de erro, melhora
            if data.get("sub_intencao") in (
                None,
                "",
                "geral",
                "erro_parse",
                ...
                data["sub_intencao"] = "entender_resultado_farol"

        # b) Termos clássicos de avaliação/nota
        if any(
            termo in q_norm
            for termo in [
                "nota da avaliacao",
            ...
            if data.get("modulo") != "avaliacao":
                logging.debug(
                    "Override heurístico: detectado termo de nota/avaliação, "
                    "forçando modulo='avaliacao'."
                )
            data["modulo"] = "avaliacao"
            if data.get("sub_intencao") in (
                ...
                data["sub_intencao"] = "entender_resultado"
