...
#
# MODIFICAÇÕES (ALINHADAS AO ÍNDICE 'atribuicao'):
# 1. _embedding_or_none:    Fallback da OpenAI pública usa "text-embedding-3-large".
# 2. _get_meta_from_hit:    Nova função helper para extrair metadados (doc_title, norma_tipo, etc).
# 3. Funções de extração:   (_harvest_semantic_evidence_local, _extract_quotes, _find_explicit_policy_statements, 
#                           _add_query_doc_cooccurrence) agora chamam _get_meta_from_hit.
# 4. _call_llm_summarize:   Prompt do LLM foi reformatado para incluir os metadados ricos na
#                           linha de "Contexto:" de cada trecho, melhorando a resposta do RAG.
#
# **** CORREÇÕES APLICADAS (BASEADO NO HISTÓRICO DE DEBUG) ****
# 1. (Bug 1) _embedding_or_none (L.194): API version corrigida para AOAI_API_VERSION.
# 2. (Bug 2) _text_search (L.574): queryLanguage="pt-BR" comentado.
# 3. (Bug 3) _call_llm_summarize (L.372, L.413): Prompts rígidos substituídos por flexíveis.
# 4. (Bug 4) _normalize_hit (L.620): Lógica de score corrigida para incluir vectorSearchScore.
# 5. (Bug 4) _filter_hits_by_relevance (L.630): Adicionado filtro de relevância (de v_old).
# 6. (Bug 4) handle_search_request (L.981, L.1003): Adicionada chamada de filtro e removido re-scoring por keyword.
...
# **** CORREÇÃO BUG 4 (Filtro) ****
# Adicionando o filtro de relevância (afrouxado)
...
# --- NOVA FUNÇÃO HELPER ---
def _get_meta_from_hit(h: Dict[str, Any]) -> Dict[str, Any]:
    """Extrai os metadados ricos do hit de busca para o RAG."""
    return {
        "doc_title": h.get("doc_title"),
        "norma_tipo": h.get("norma_tipo"),
        "data_publicacao": h.get("data_publicacao"),
        "fase_processo": h.get("fase_processo"),
        "publico_alvo": h.get("publico_alvo"),
        "conhecimento": h.get("conhecimento"),
    }
# --- FIM DA NOVA FUNÇÃO ---
...
def _call_llm_summarize(question: str, quotes: List[Dict[str, str]]) -> Optional[str]:
    ...
    for i, q in enumerate(quotes, start=1):
        ...
        meta_parts = [
            f"Fonte: {_filename_from_source(src)}" if src else "Fonte: desconhecida",
            f"Título: {q.get('doc_title')}" if q.get('doc_title') else None,
            f"Tipo: {q.get('norma_tipo')}" if q.get('norma_tipo') else None,
            # Pega apenas a data YYYY-MM-DD
            f"Data: {q.get('data_publicacao', 'T')[0:10]}" if q.get('data_publicacao') else None,
            f"Fase: {q.get('fase_processo')}" if q.get('fase_processo') else None,
            f"Público: {q.get('publico_alvo')}" if q.get('publico_alvo') else None
        ]
        meta_str = " | ".join(p for p in meta_parts if p)
        # --- FIM DA NOVA LÓGICA ---

        prepared.append({"i": i, "meta": meta_str, "text": txt})
    # --- FIM DO BLOCO MODIFICADO ---

    # **** CORREÇÃO BUG 3 (Prompt Flexível) ****
    system_msg = (
    ...
    # Instrução adicional condicionada: só comece com 'Sim.'/'Não.' quando a pergunta for do tipo fechada
    if is_yes_no:
        yesno_instr = (
    ...
    # **** CORREÇÃO BUG 3 (Removendo Regra Rígida) ****
    user_msg_instructions = (
        ...
        # REMOVIDAS AS REGRAS 5 E 6 ANTIGAS (SOBRE 'Informação não encontrada')
    )
...
def _text_search(query: str, topk: int, force_semantic: bool = False, return_raw: bool = False) -> Any:
    ...
    if semantic_on:
        base_payload.update({
            "queryType": "semantic",
            # **** CORREÇÃO BUG 2 (Conflito de Acentos) ****
            # "queryLanguage": "pt-BR", 
            "answers": "extractive",
            "answersCount": 1,
            "captions": "extractive",
...
def _normalize_hit(h: Dict[str, Any]) -> Dict[str, Any]:
    # **** CORREÇÃO BUG 4a (Lógica de Score) ****
    # PRIORITIZE scores: 1. Semantic, 2. Vector, 3. Keyword
    score = (h.get("@search.rerankerScore") / 4.0) if h.get("@search.rerankerScore") else \
            (h.get("@search.vectorSearchScore") or h.get("@search.score") or 0.0)
    
    return {
        "id": h.get("id"),
        "score": score, # Usa o score combinado
        "text": _clean_text(h.get("text") or ""),
        "source_file": h.get("source_file"),
        "id_original": h.get("id_original"),
        # --- Propaga metadados para _extract_quotes ---
        **_get_meta_from_hit(h) 
    }

# **** CORREÇÃO BUG 4b (Adicionando Filtro) ****
def _filter_hits_by_relevance(nhits: List[Dict[str, Any]], min_score: float) -> List[Dict[str, Any]]:
    """Filtra hits por score normalizado. A lógica de 'perdão' é de v_old."""
    if not nhits:
        return []
    strong = [h for h in nhits if (h.get("score") or 0.0) >= min_score]
    if strong:
        return strong
    # Lógica de 'perdão': se nada for forte, pegue os 5 melhores scores (mesmo que baixos).
    ...
    for h in hits:
        ...
        if maybe_def:
            ...
            if 40 <= len(snip) <= 420 and snip not in seen and _has_key_term(snip):
                # --- MODIFICADO ---
                quotes.append({"source": h.get("source_file") or h.get("id_original") or "", "text": snip, **_get_meta_from_hit(h)})
                # --- FIM DA MODIFICAÇÃO ---
                seen.add(snip)
        ...
        for i, sc in scored:
            ...
            if not _has_key_term(snip):
                if not any(tok.upper() in snip.upper() for tok in q_tokens):
                    continue
            # --- MODIFICADO ---
            quotes.append({"source": src, "text": snip, **_get_meta_from_hit(h)})
            # --- FIM DA MODIFICAÇÃO ---
            seen.add(snip)
            taken += 1
            ...
            for pat in patterns:
                ...
                if m:
                    ...
                    if ctx and not any(ctx == x["text"] for x in found):
                        # --- MODIFICADO ---
                        found.append({"source": src, "text": ctx, **_get_meta_from_hit(h)})
                        # --- FIM DA MODIFICAÇÃO ---
                    break
...
# (Substitua a partir da linha 1141)
...
def _answer_from_quotes(query: str, quotes: List[Dict[str, str]], sources: List[str]) -> Dict[str, Any]:
    def _is_truncated_text(t: str) -> bool:
        ...
        # --- INÍCIO DA MODIFICAÇÃO (Correção 2) ---
        # Ensina a função que (s/n) é um final válido
        if t.endswith("(s/n)") or t.endswith("(s/n)..."):
            return False
        # --- FIM DA MODIFICAÇÃO ---
    ...
    # --- INÍCIO DA MODIFICAÇÃO (FLUXO LÓGICO CORRIGIDO) ---

    summary_clean = (summary or "").strip()

    # CASO 1: A resposta é PURA "Informação não encontrada".
    # Se a resposta começar com "Informação não encontrada", retorne a estrutura limpa IMEDIATAMENTE.
    if summary_clean.startswith("Informação não encontrada") or summary_clean.startswith("Não tenho essa informação"):
        logging.info("Summary is 'Not Found'. Returning clean response structure.")
        return {
            "text": summary_clean.rstrip('.'), # Garante o texto limpo
            "sources": [], # Fontes limpas
            "meta": {"tried_completion": False, "quotes_count": len(quotes), "issues": ["llm_said_no_evidence"]}
        }

    # CASO 2: A resposta é MISTA (ex: "...[1].[4] Informação não encontrada...").
    # Se chegamos aqui, a resposta NÃO começa com "Não encontrada", mas PODE conter
    # o texto misturado. Vamos limpar.
    
    # Regex mais robusto: captura qualquer combinação de citações [N], pontos e espaços
    #clean_pattern = r'[\s.,\[\]\d]*\b(Informação não encontrada|Não tenho essa informação)\b[\.]*'
    
    clean_pattern = r'[\W\d_]*\b(Informação não encontrada|Não tenho essa informação)\b[\.]*'
    if "Informação não encontrada" in summary or "Não tenho essa informação" in summary:
        summary_before_clean = summary
        
        # Aplicamos a limpeza com o novo regex
        #summary = re.sub(clean_pattern, '', summary, flags=re.I).strip()
        summary = re.sub(clean_pattern, '', summary).strip()
        # Limpeza de lixo/pontuação que sobrou
        summary = re.sub(r'(\[\d+\])\s*\.', r'\1.', summary) # "resposta [1] ." -> "resposta [1]."
        summary = summary.rstrip(' .,') # Remove lixo no final
        
        # Se a resposta ficou vazia após a limpeza, retorne o 'not found' limpo
        # (Isso é um fallback, não deve acontecer se o CASO 1 acima funcionar)
        if not summary:
            summary = "Informação não encontrada" # Fallback
            logging.warning("Summary was mixed but became empty after cleaning. Returning 'Not Found'.")
            return {
                "text": summary, 
                "sources": [], 
                "meta": {"tried_completion": False, "quotes_count": len(quotes), "issues": ["llm_said_no_evidence"]}
        ...
        logging.info("Cleaned mixed 'Not Found' from summary. Original: '%s' | Cleaned: '%s'", summary_before_clean, summary)
    ...
    if _is_truncated_text(summary):
        ...
        # Só adiciona "Quer mais detalhes" se o LLM já não o fez
...
# (O resto do seu script continua abaixo da linha 1226)
    ...
    for h in nhits:
        ...
        if len(matched_tokens) >= min_tokens_match:
            ...
            if ctx_clean and len(ctx_clean) >= 30 and ctx_clean not in seen_texts:
                ...
                # --- MODIFICADO ---
                added.append({"source": src, "text": ctx_clean, **_get_meta_from_hit(h)})
                # --- FIM DA MODIFICAÇÃO ---
        ...
        if found_pair_ctx:
            src = h.get("source_file") or h.get("id_original") or h.get("id") or ""
            # --- MODIFICADO ---
            added.append({"source": src, "text": found_pair_ctx, **_get_meta_from_hit(h)})
            # --- FIM DA MODIFICAÇÃO ---
...
def handle_search_request(body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _harvest_semantic_evidence_local(sem_json_local: Any) -> List[Dict[str, str]]:
        ...
        for a in ans:
            ...
            if 40 <= len(snip) <= 420:
                # O hit de @search.answers não tem metadados de documento, 
                # então não podemos chamar _get_meta_from_hit aqui.
        ...
        for v in vals:
            ...
            for c in caps:
                ...
                if 40 <= len(snip) <= 420:
                    # --- MODIFICADO ---
                    quotes_local.append({"source": src, "text": snip, **_get_meta_from_hit(v)})
                    # --- FIM DA MODIFICAÇÃO ---
        seen = set()
    ...
    # **** CORREÇÃO BUG 4c (Aplicando Filtro) ****
    nhits = _filter_hits_by_relevance(nhits, RELEVANCE_THRESHOLD_HITS)
    ...
    # **** CORREÇÃO BUG 4d (Removendo Re-scoring) ****
    # O bloco abaixo re-pontuava os resultados da busca vetorial/semântica
    # usando apenas keywords, o que quebrava a lógica.
...
def http_search_trigger(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        # 1. Obter o corpo (body) da requisição
        body = req.get_json()
        if not body:
            return func.HttpResponse(
                 "Por favor, passe um corpo JSON com uma 'query'.",
                 status_code=400
            )

        # 2. Chamar sua função de lógica principal existente
        result = handle_search_request(body)

        # 3. Retornar o resultado da sua função
        return func.HttpResponse(
        ...
        logging.exception(f"Erro no http_search_trigger: {e}")
        return func.HttpResponse(
...
if __name__ == "__main__":
    # (Seu código de teste local permanece o mesmo)
    test_query = _safe_str_env("TEST_QUERY", "O que é uma Demanda nova?")
    # ... etc ...